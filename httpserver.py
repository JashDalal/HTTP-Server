...
def parseRequest(client_input):
	...
	try:
			...
			with open('GET_Parameters.csv', 'w') as csvfile:
				...
				if(params.find("&") != -1):
					...
					for param in params_list:	 
						var = param.split("=")
						key = urllib.parse.unquote_plus(var[0]) #Urllib.parse is used for parsing the encoded URL sent by the client
		...
		if(version == "HTTP/1.1"):
			...
			i += 1
		#The headers of the first line contains the method, path of the file and the HTTP protocol and the second line contains HOST.
		#Now accepting other headers
		...
		if(method == "PUT"):
			#Stores file in the string fileput. This will be a text file because it has to be a part of the request body
			fileput = ''
			while(i < len(req)):
	...
	#In case of an error, response headers will be returned
		...
		server_name = 'Umash/1.0.0 (Ubuntu)'
		connection = ''
		...
		headers = dict([('Date', date), ('Server', server_name), ('Connection', connection), ('Content-Type', content_type), ('Version', version), ("Method", method)])
		
		#Handle 400: Bad Request
		if((method != "GET" or method != "POST" or method != "HEAD" or method != "OPTIONS") or (version != "HTTP/1.1" or version != "HTTP/1.0") or (version == "HTTP/1.1" and host_name == "")):
			headers["Error"] = 400
...
def processMethod(req_headers):
	...
	if((method.upper() == "GET" and method != "GET") or (method.upper() == "POST" and method != "POST") or (method.upper() == "PUT" and method != "PUT") or (method.upper() == "POST" and method != "POST") or (method.upper() == "HEAD" and method != "HEAD") or (method.upper() == "OPTIONS" and method != "OPTIONS") or (method.upper() == "DELETE" and method != "DELETE") or (method == "CONNECT") or (method == "TRACE") or (method == "PATCH")):
	...
	if(error):
		if(error == 400):
			...
			myfile = open('Error/400.html', 'rb')
		if(error == 403):
			status_line = version + " 403 Forbidden"
			myfile = open('Error/403.html', 'rb')
		if(error == 404):
			status_line = version + " 404 Not Found"
			myfile = open('Error/404.html', 'rb')
		if(error == 405):
			status_line = version + " 405 Method Not Allowed"
			myfile = open('Error/405.html', 'rb')
		if(error == 501):
			status_line = version + " 501 Method Not Implemented"
			req_headers = dict([('Date', date), ('Server', 'Umash/1.0.0 (Ubuntu)'), ('Connection', 'close'), ('Content-Type', 'text/html; charset=iso-8859-1'), ("Allow", "POST, OPTIONS, HEAD, GET"), ("Error", 501), ("Method", method), ("Version", version)])

			if(adminflag == 1):
				req_headers["Allow"] = "POST, OPTIONS, HEAD, GET, PUT, DELETE"
			myfile = open('Error/501.html', 'rb')
			
  		#Method that logs error into error_log file
		error_log(date,status_line)
		...
		return (server_input, myfile)
	
	
	#Request Header Dictionary
	path = req_headers["Path"]
	host_name = req_headers["Host"]
	#Response
	...
	if(method == "GET"):
					...
					if(months.index(last_modified_date_list[0]) <= months.index(modified_date_list[1])):
						if(months.index(last_modified_date_list[0]) == months.index(modified_date_list[1])):
							if(int(last_modified_date_list[1]) < int(modified_date_list[0])):
								...
								headers = dict([("Server", "Umash/1.0.0 (Ubuntu)"), ("Date", date)])		
		...
		headers = dict([("Server", "Umash/1.0.0 (Ubuntu)"), ("Date", date), ("Last-Modified", last_modified), ("Accept-Ranges", "bytes"), ("Content-Length", length), ("Connection", connection), ("Content-Type", content_type)])
	...
	# OPTIONS
	if(method == "OPTIONS"):
		headers = dict([("Allow", "OPTIONS, GET, HEAD, POST"), ("Server", "Umash/1.0.0 (Ubuntu)"), ("Date", date), ("Last-Modified", last_modified), ('Connection', connection), ("Content-Length", 0)])
		if(adminflag == 1):
	...
	if(method == "PUT"):
		...
		headers = dict([("Server", "Umash/1.0.0 (Ubuntu)"), ("Date", date), ("Content-Location", content_location), ("Connection", "close")])
		server_input = status_line + '\n' + '\n'.join("{0}: {1}".format(key,val) for (key,val) in headers.items()) + '\n' + '\n'
	...
	if(method == "DELETE"):
		myFile = None
		headers = dict([("Server", "Umash/1.0.0 (Ubuntu)"), ("Date", date), ("Connection", "close")])
		status_line = version + " 200 OK"
		server_input = status_line + '\n' + '\n'.join("{0}: {1}".format(key,val) for (key,val) in headers.items()) + '\n' + '\n'
...
def error_log(date, status_line):
	Message = date + " " + status_line;
	appendFile = open("error_log", "a+")
